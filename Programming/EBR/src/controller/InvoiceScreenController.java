...
public class InvoiceScreenController extends BaseController {
	...
	public PaymentTransaction makeTransaction(int amount, CreditCard card)
		...
		if (amount >= 0) {
			return this.refund(amount, "refund", card.getCardNum(), card.getCardOwner(), card.getExpDate(),
					Integer.toString(card.getSecurityCode()));

		} else {
			return this.pay(-amount, "pay over amount", card.getCardNum(), card.getCardOwner(), card.getExpDate(),
					Integer.toString(card.getSecurityCode()));
		}
	}
	...
	public void returnBikeProcessing(Invoice invoice, PaymentTransaction returnTransaction) {
		...
		SessionManager.getInstance().endSession(SessionManager.getInstance().getSessionById(invoice.getSessionId()),
				returnTransaction);
