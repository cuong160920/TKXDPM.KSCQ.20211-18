...
//        refreshSessionsList();
	 ...
	 * @param returnTransaction {@link PaymentTransaction} transaction to refund
	 *                          deposit after deducting rental fee
	...
	public int endSession(Session session, PaymentTransaction returnTransaction) {
		...
		String SQL = "UPDATE session " + "SET (end_time, return_transactionid) = (?, ?::uuid) " + "WHERE id = ?::uuid ";
...
//        this.refreshSessionsList();
	...
	private void refreshSessionsList() {
		...
		// Get All Rows
		try (Statement stmt = EBRDB.getConnection().createStatement(); ResultSet rs = stmt.executeQuery(SQL)) {
			while (rs.next()) {
				...
				if (return_transactionid == null || end_time == null) {
					session = new Session(id, BikeManager.getInstance().getBikeById(bike_id),
	...
	private int pauseSession(Session session) {
		int realRentingTime = (int) (session.getLastRentTimeBeforeLock()
				+ Utils.minusLocalDateTime(session.getLastResumeTime(), LocalDateTime.now()));
		session.setLastRentTimeBeforeLock(realRentingTime);
		session.setActive(false);

		String SQL = "UPDATE session " + "SET (last_rent_time_before_lock, active) = (?, ?) " + "WHERE id = ?::uuid ";
	...
	private int resumeSession(Session session) {
		...
		String SQL = "UPDATE session " + "SET (active, last_resume_time) = (?, ?) " + "WHERE id = ?::uuid ";
