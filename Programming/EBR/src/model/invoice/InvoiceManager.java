...
public class InvoiceManager {
	...
	/**
	 * This method is for create new Invoice using session id and update it to the
	 * DB
	 * 
	 ...
	 * @param total_charge total charge of the session corresponding to chosen
	 *                     invoice
	...
	public int finalInvoice(Invoice invoice, int total_charge) {
		...
		String SQL = "UPDATE invoice " + "SET (total_charge) = row(?) " + "WHERE id = ?::uuid ";
	...
	private String updateInvoiceHistory(Invoice newInvoice) {
		String SQL = "INSERT INTO invoice(session_id) " + "VALUES(?::uuid)";
		String id = "";
	...
	public void refreshInvoiceHistory() {
		...
		try (Statement stmt = EBRDB.getConnection().createStatement(); ResultSet rs = stmt.executeQuery(SQL)) {
			while (rs.next()) {
				...
				if (total_charge == 0) {
					invoice = new Invoice(id, session_id);
				} else {
					invoice = new Invoice(id, session_id, total_charge);
				}
	...
	public Invoice getInvoiceBySessionId(String sessionId) {
		for (Invoice invoice : invoiceHistory) {
			if (invoice.getSessionId() == sessionId)
				return invoice;
		}
		return null;
	}
