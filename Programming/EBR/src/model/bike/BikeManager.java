...
/**
 * Model to manage all bikes and handle database connections that related to
 * bikes' info
 *
...
public class BikeManager {
	...
	public ArrayList<Bike> getBikeListInDock(String dockId) {
		...
		try (Connection conn = EBRDB.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
	...
	private StandardBike createStandardBike(ResultSet resultSet) throws SQLException {
		StandardBike bike = new StandardBike(resultSet.getString("id"), resultSet.getInt("barcode"),
				resultSet.getInt("value"), resultSet.getInt("rental_fees"));
	...
	private TwinBike createTwinBike(ResultSet resultSet) throws SQLException {
		TwinBike bike = new TwinBike(resultSet.getString("id"), resultSet.getInt("barcode"), resultSet.getInt("value"),
	...
	private StandardElectricalBike createStandardElectricalBike(ResultSet resultSet) throws SQLException {
		StandardElectricalBike bike = new StandardElectricalBike(resultSet.getString("id"), resultSet.getInt("barcode"),
				resultSet.getInt("value"), resultSet.getInt("rental_fees"));
		bike.setPairOfPedals(resultSet.getInt("pedal_num"));
		bike.setSaddle(resultSet.getInt("saddle_num"));
		bike.setRearSeat(resultSet.getInt("rear_seat_num"));
	...
	private TwinElectricalBike createTwinElectricalBike(ResultSet resultSet) throws SQLException {
		TwinElectricalBike bike = new TwinElectricalBike(resultSet.getString("id"), resultSet.getInt("barcode"),
				resultSet.getInt("value"), resultSet.getInt("rental_fees"));
		bike.setPairOfPedals(resultSet.getInt("pedal_num"));
		bike.setSaddle(resultSet.getInt("saddle_num"));
		bike.setRearSeat(resultSet.getInt("rear_seat_num"));
	...
	public void updateDockOfBike(Bike bike, String dockId) {
		String SQL = "UPDATE bike " + "SET dock_id = ?::uuid " + "WHERE id = ?::uuid";
		if (dockId == null || dockId.equals("")) {
			SQL = "UPDATE bike " + "SET dock_id = NULL " + "WHERE id = ?::uuid";
		}
