...
public class CreditCardManager {
	...
	public CreditCard getCardById(String cardId) {
		// query the card
		String SQL = "SELECT * FROM card " + "WHERE id = ?::uuid";
		...
		try (Connection conn = EBRDB.getConnection(); PreparedStatement pstmt = conn.prepareStatement(SQL)) {
			...
			if (rs.next()) {
				...
				return new CreditCard(cardId, rs.getString("card_num"), rs.getString("card_owner"), 000,
	...
	public CreditCard getCardByCardNumber(String cardNumber) {
		// query the card
		String SQL = "SELECT * FROM card " + "WHERE card_num = ? " + "ORDER BY id " + "FETCH FIRST ROW ONLY";
		...
		try (Connection conn = EBRDB.getConnection(); PreparedStatement pstmt = conn.prepareStatement(SQL)) {
			...
			if (rs.next()) {
				...
				return new CreditCard(rs.getString("id"), rs.getString("card_num"), rs.getString("card_owner"), sc,
						rs.getString("exp_date"));
			}
		} catch (SQLException ex) {
			ex.printStackTrace();
	...
	public String saveCreditCard(CreditCard creditCard) {
		String SQL = "INSERT INTO card(card_num, card_owner, security_code, exp_date) " + "VALUES (?, ?, ?, ?)";
