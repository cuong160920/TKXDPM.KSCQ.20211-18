...
public class PaymentConfirmationScreenHandler extends BaseScreenHandlerWithTransactionPopup implements Initializable {
	 ...
	 */
	public PaymentConfirmationScreenHandler(Stage stage, String screenPath, PaymentScreenController controller)
			throws IOException {
	...
	private void handleRentingConfirmation() throws IOException {
		...
		try {
			PaymentTransaction rentTransaction = this.controller.payDeposit(this.controller.getBike().getDeposit(),
					contents, this.controller.getCardInfo().get("cardNumber"),
					this.controller.getCardInfo().get("cardOwner"), this.controller.getCardInfo().get("expDate"),
					this.controller.getCardInfo().get("securityCode"));
	...
	private void transitionToSessionScreen(PaymentTransaction rentTransaction) {
		try {
			...
			Session session = SessionManager.getInstance().createSession(this.controller.getBike(),
					rentTransaction.getCard(), rentTransaction);
	...
	private void saveTransaction(PaymentTransaction rentTransaction) throws IOException {
		CreditCard card = new CreditCard(this.controller.getCardInfo().get("cardNumber"),
				this.controller.getCardInfo().get("cardOwner"),
				Integer.parseInt(this.controller.getCardInfo().get("securityCode")),
				this.controller.getCardInfo().get("expDate"));
