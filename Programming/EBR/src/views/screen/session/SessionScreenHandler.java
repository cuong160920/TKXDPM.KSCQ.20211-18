...
public class SessionScreenHandler extends BaseScreenHandler implements Initializable {
	 ...
	 * @param controller {@link SessionScreenController sessionScreenController}
	 *                   Controller of this screen
	 * @throws IOException IO error
	 */
	public SessionScreenHandler(Stage stage, String screenPath, Session session, SessionScreenController controller)
			throws IOException {
		...
		Timeline timeline = new Timeline(new KeyFrame(Duration.seconds(1), e -> {
			if (session.isActive()) {
				...
				setSessionLengthText();
				sessionRentingFee.setText(
						this.getBController().calculateCurrentRentingFees(this.session) + " " + Configs.CURRENCY);
			}
		}));
	...
	private void goToDockSelection() throws IOException {
		try {
			ReturningDockSelectionHandler returningDockSelectionHandler = new ReturningDockSelectionHandler(this.stage,
					Path.RETURNING_DOCK_SELECTION_SCREEN_PATH, new ReturningDockSelectionController(), session);
