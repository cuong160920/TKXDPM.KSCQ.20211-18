...
public class ReturningDockSelectionHandler extends BaseScreenHandler implements Initializable {
	 ...
	 */
	public ReturningDockSelectionHandler(Stage stage, String screenPath,
			ReturningDockSelectionController returningDockSelectionController, Session session) throws IOException {
	...
	public void displayDockList() {
		...
		try {
			for (Dock dock : dockList) {
				ReturningDockListItemHandler dockListItem = new ReturningDockListItemHandler(
						Path.RETURNING_DOCK_LIST_ITEM_PATH, this);
	...
	void returnBikeBtnListener(MouseEvent e) {
		if (dock != null) {
			...
			try {
				invoiceScreenHandler = new InvoiceScreenHandler(this.stage, Path.INVOICE_SCREEN_PATH, invoice,
